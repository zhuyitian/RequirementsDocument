...
const val SHARE_CONTENT_TEXT = "shareContentText"
const val GET_COUNTRY_CODE = "getCountryCode"
...
class xxx {
    ...
    override fun callMethod(data: String): String? {
	    ...
	    GET_COUNTRY_CODE -> {
	        return countryCode
		//关于获取countryCode方法，请参考 `api文档.md` 中的 `获取countryCode方法`。
	    }
            else -> {
            }
        }
...
/**
 * 分享图片
 * 提供测试的base64Str详情见image_test_base64.txt文件
 *
 *（更新日期：2023-03-24）
 */
private fun shareImage(base64Str: String) {
    //TODO 需要实现将H5给的base64Str转换成图片并分享到三方的功能
}
 ...
 *（更新日期：2023-03-24）
 */
private fun saveImage(base64Str: String) {
    //TODO 需要实现将H5给的base64Str转换成图片并保存到本地相册功能。
    //保存成功后加个Toast提示一下，提示内容为（默认为英语：Success 或者葡萄牙语：Sucesso；备注：判断 countryCode == "BR"时需展示葡萄牙语）
}
 ...
 *（更新日期：2023-03-24）
 */
private fun shareContentText(textString: String) {
    //TODO 需要实现将H5给的textString分享到三方的功能
}
